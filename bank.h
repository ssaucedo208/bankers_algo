...
public:
  ...
  bool is_safe(int id, const ext_vector<int>& req) {   // TODO:  implement check for request not leading to deadlock
    ...
    ext_vector<int> availresources = this->avail;
    availresources -= req;
    ...
    {
      if(availresources >= this->customers[i]->create_req())
      {
        // roll back request
        grant = true;
      }
      else if(availresources < req)
      {
        grant = false;
        continue;
      }
    ...
    // return anaswer;
    return grant;   // TODO: calculate if it is really safe
  }
